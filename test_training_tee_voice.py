#!/usr/bin/env python3
"""
test_training_tee_voice.py — Tests Training Tee's encoder→decoder pipeline on in-domain queries.
Bypasses Ollama entirely. Pure Training Tee: encode query → cosine match → decode response.
...
print("[Test] Loading Training Tee…", flush=True)
bridge = DataGBridge.get()
if not bridge.ready:
    print("[Test] FAILED — Training Tee's substrate didn't load")
    sys.exit(1)
print(f"[Test] Training Tee online — {len(bridge.substrate.methodology_cells)} cells\n")


# ── In-domain test queries ────────────────────────────────────────────────────
# These are things Training Tee should actually know about
QUERIES = [
    # Identity
    "who are you",
    ...
    "can you learn",
    # Reasoning
    "how would you approach a new sale?",
    "what can you do to close a sale",
    "can you sell me something?",
    "what do you do when you're stuck",
    "how do you make architecture decisions",
    # Code
    "show me a sales pitch",
    "how do you close the deal",
    "what is a decorator",
    # Meta / conversational
    "hi",
    "can you tell me about your main product?",
]

W = 72
print("=" * W)
print("  TRAINING_TEE VOICE TEST -- Encoder -> Substrate -> Decoder")
...
for i, query in enumerate(QUERIES, 1):
    ...
    # Display results
    print(f"\n  TRAINING_TEE (field_read, {dec_ms:.0f}ms):")
    if decoded:
...
if decoder_empty <= 2 and field_empty <= 2:
    print("  ✅ Training Tee's voice is working — decoder is generating text from latent space")
